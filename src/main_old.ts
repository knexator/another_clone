...
abstract class ButtonTarget {
    abstract button_on(state: GameState): void;
    abstract button_off(state: GameState): void;
    abstract clone(): ButtonTarget;
    abstract draw(): void;
}
...
class TwoStateWall extends ButtonTarget {
    ...
    button_on(state: GameState): void {
        if (!this.extended) {
            this.extended = moveThing(state, this.pos, this.dir);
        }
    }

    button_off(state: GameState): void {
    ...
    clone(): TwoStateWall {
        return new TwoStateWall(this.pos.clone(), this.dir.clone(), this.active);
    }
}
...
let robot_delay = 5;
let FLOORS_UPDATE_EVERY_STEP = false;
...
function drawGameState(state: GameState) {
    ...
    state.targets.forEach(target => {
        target_sprite.position.set((target.x + 1) * TILE_SIZE, (target.y + 1) * TILE_SIZE);
...
function update() {
    ...
    if (time_offset > 0) {
        ...
        time_offset = Math.max(0, time_offset - Shaku.gameTime.delta! * (Math.abs(cur_turn - selected_tape_pos) + 1) / (miniturn_duration * all_deltas[cur_turn].intermediate_states.length))
    } else if (time_offset < 0) {
        drawDeltaState(all_deltas[cur_turn - 1], 1 + time_offset);
        time_offset = Math.min(0, time_offset + Shaku.gameTime.delta! * (Math.abs(cur_turn - selected_tape_pos) + 1) / (miniturn_duration * all_deltas[cur_turn - 1].intermediate_states.length))
    } else {
